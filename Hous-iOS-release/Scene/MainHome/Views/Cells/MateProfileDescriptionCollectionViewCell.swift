...
final class MateProfileDescriptionCollectionViewCell: UICollectionViewCell {
  ...
  private let profileDescriptionCollectionView: UICollectionView = {
    ...
    layout.itemSize = CGSize(width: 170, height: 102)

    let collectionView = UICollectionView(frame: .zero, collectionViewLayout: layout)
    collectionView.register(cell: MateProfileDescInnerCollectionViewCell.self)
  ...
  private func bind() {
    let personalityAttrDescriptionObservable:
    Observable<[PersonalityAttributeDescription]> = Observable.of(personalityAttributeDescriptions)
    ...
    personalityAttrDescriptionObservable
      .bind(to: profileDescriptionCollectionView.rx.items) {
        ...
        guard let cell = self.profileDescriptionCollectionView
          .dequeueReusableCell(
            withReuseIdentifier: MateProfileDescInnerCollectionViewCell.className,
            for: indexPath) as?
           MateProfileDescInnerCollectionViewCell
