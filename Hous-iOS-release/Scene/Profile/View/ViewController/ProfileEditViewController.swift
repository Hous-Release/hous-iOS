...
final class ProfileEditViewController: UIViewController {
  ...
  private var nameTextField: ProfileEditTextField = {
    ...
    let attrPlaceholder = NSAttributedString(
      string: "닉네임",
      attributes: [NSAttributedString.Key.foregroundColor: Colors.g5.color])
  ...
  private var birthdayTextField: ProfileEditTextField = {
    ...
    let attrPlaceholder = NSAttributedString(
      string: "생년월일",
      attributes: [NSAttributedString.Key.foregroundColor: Colors.g5.color,
                   NSAttributedString.Key.font: Fonts.SpoqaHanSansNeo.medium.font(size: 16)])
...
extension ProfileEditViewController {
  ...
  private func bind() {
    ...
    birthdayTextField.birthdayPublicButton.rx.tap
      ...
      .bind(onNext: { [weak self] in
        guard let self = self else { return }
        self.actionDetected.onNext(
          .birthdayPublicEdited(isPublic: !self.birthdayTextField.birthdayPublicButton.isSelected))
      })
      .disposed(by: disposeBag)
  ...
  private func render() {
    [nameTextField, birthdayTextField, mbtiTextField, jobTextField]
      .forEach {profileEditStackView.addArrangedSubview($0)}
  ...
  private func isAllOptionValid() -> Bool {
    ...
    where textField.isFloatingErrorMessage {
      return false
    }
