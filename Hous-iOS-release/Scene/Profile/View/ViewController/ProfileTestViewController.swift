...
final class ProfileTestViewController: BaseViewController, LoadingPresentable {
  ...
  private lazy var testNavigationStackView = UIStackView(
    arrangedSubviews: [backwardButton, testCountLabel, forwardButton]).then {
    $0.axis = .horizontal
    $0.spacing = 30
  }
...
extension ProfileTestViewController {
  ...
  private func bind() {
    ...
    output.profileTestData
      ...
      .bind(to: testCollectionView.rx.items) {
            (_: UICollectionView, index: Int, element: ProfileTestItemModel) in
            let indexPath = IndexPath(row: index, section: 0)
            guard let cell =
                    self.testCollectionView.dequeueReusableCell(
                      withReuseIdentifier: TestCollectionViewCell.className,
                      for: indexPath)
                    as? TestCollectionViewCell else { print("Cell Loading ERROR!"); return UICollectionViewCell()}
  ...
  private func finishTest() {
    ...
    destinationViewController.profileTestSaveData = ProfileTestSaveModel(
      selectedData: viewModel.selectedData, questionType: viewModel.questionTypes)
