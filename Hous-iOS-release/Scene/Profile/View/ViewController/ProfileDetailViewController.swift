  ...
  private func bind() {
    ...
    output.profileDetailModel
      ...
      .bind(to: profileDetailCollectionView.rx.items) {
        (_: UICollectionView, index: Int, element: ProfileDetailModel) in
        let indexPath = IndexPath(row: index, section: 0)

        switch indexPath.row {
        case 0:
          guard let cell =
                  self.profileDetailCollectionView.dequeueReusableCell(
                    withReuseIdentifier: ProfileDetailImageCollectionViewCell.className,
                    for: indexPath) as? ProfileDetailImageCollectionViewCell
        ...
        case 1:
          guard let cell =
                  self.profileDetailCollectionView.dequeueReusableCell(
                    withReuseIdentifier: ProfileDetailTextCollectionViewCell.className,
                    for: indexPath) as? ProfileDetailTextCollectionViewCell
          else { print("Cell Loading ERROR!"); return UICollectionViewCell()}
          cell.bind(element)
          return cell
        case 2:
          guard let cell =
                  self.profileDetailCollectionView.dequeueReusableCell(
                    withReuseIdentifier: ProfileDetailRecommendCollectionViewCell.className,
                    for: indexPath) as? ProfileDetailRecommendCollectionViewCell
          else { print("Cell Loading ERROR!"); return UICollectionViewCell()}
          cell.bind(element)
          return cell
        default:
