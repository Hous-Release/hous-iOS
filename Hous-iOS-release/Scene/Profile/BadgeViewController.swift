...
class BadgeViewController: BaseViewController, LoadingPresentable {
  ...
  private let badgeCollectionView = UICollectionView(
    frame: .zero,
  ...
  RxCollectionViewSectionedReloadDataSource<SectionOfProfile>
    .ConfigureCell = { [unowned self] (_, _, indexPath, item) -> UICollectionViewCell in
  ...
  private lazy var dataSource = RxCollectionViewSectionedReloadDataSource<SectionOfProfile>(
  ...
  private func setBadgeCollectionView() {
    ...
    badgeCollectionView.register(
      RepresentingBadgeCollectionViewCell.self,
      forCellWithReuseIdentifier: RepresentingBadgeCollectionViewCell.className)

    badgeCollectionView.register(BadgeCollectionViewCell.self,
                                 forCellWithReuseIdentifier: BadgeCollectionViewCell.className)
  }
...
extension BadgeViewController: UICollectionViewDelegateFlowLayout {
  func collectionView(_ collectionView: UICollectionView,
    ...
    if indexPath.section == 0 {
      return CGSize(width: view.window?.windowScene?.screen.bounds.width ?? 375,
                    height: (view.window?.windowScene?.screen.bounds.width ?? 375)
                    * (180 / (view.window?.windowScene?.screen.bounds.width ?? 375)))
    }
...
extension BadgeViewController {
  ...
  func configRepresentingBadgeCell(
    ...
    indexPath: IndexPath) -> UICollectionViewCell {
      guard let cell = self.badgeCollectionView.dequeueReusableCell(
        withReuseIdentifier: RepresentingBadgeCollectionViewCell.className,
        for: indexPath) as? RepresentingBadgeCollectionViewCell else { return UICollectionViewCell() }
  ...
  func configBadgesCell(
    ...
    indexPath: IndexPath) -> UICollectionViewCell {
      guard let cell = self.badgeCollectionView.dequeueReusableCell(
        withReuseIdentifier: BadgeCollectionViewCell.className,
        for: indexPath) as? BadgeCollectionViewCell else { return UICollectionViewCell() }
